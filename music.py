...
class Note(object):
    def __init__(self, value=0, octave=5, dur=0.25, volume=100):
        if isinstance(value, str):
            self.value, self.octave, self.dur, self.volume = pyknonfinal.notation.parse_note(value)
        else:
    ...
    def __init__(self, args=None):
        if isinstance(args, str):
            if args.startswith("file://"):
                ...
                note_lists = pyknonfinal.notation.parse_notes(self._parse_score(filename))
            else:
                note_lists = pyknonfinal.notation.parse_notes(args.split())
